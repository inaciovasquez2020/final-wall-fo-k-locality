...
This follows from standard Ehrenfeucht--Fra\"{\i}ss\'e game bounds.
...
All results beyond this point depend only on the following assumptions.

\begin{enumerate}[label=\textbf{A\arabic*.}]
\item \textbf{Overlap Rigidity.}  
There exist constants $R_0,m$ such that if a graph is FO$^{4}_{R_0}$--locally homogeneous,
then it does not admit more than $m$ linearly independent cycles whose $R_0$--supports
overlap at a single vertex.

\item \textbf{Divergence Implies Independence.}  
The FO$^{4}$ predicate $\operatorname{Diverge}_{R_0}$ correctly witnesses linear
independence in the $\mathbb F_2$ cycle space.
\end{enumerate}
...
\textbf{Status.}  
This lemma depends only on Assumptions A1 and A2.
